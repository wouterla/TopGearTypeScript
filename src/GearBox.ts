 ...
 */
class GearBox {
    public current_gear: number = 0;
    private last_rpm: number = 0;

    private readonly _HIGH_RMP = 2000;
    private readonly _LOW_RPM = 500;
    private readonly _NEUTRAL = 0;
    private readonly _LOWEST_GEAR = 1;
    private readonly _TOP_GEAR = 6;

    public changeGear(current_rpm: number): number {

        if (this.current_gear < this._NEUTRAL) {
            // Don't shift out of reverse!
            this.last_rpm = current_rpm;
        } else {
            if (this.current_gear > this._NEUTRAL) {
                if (current_rpm > this._HIGH_RMP && this.current_gear < this._TOP_GEAR) {
                    this.current_gear++;
                } else if (current_rpm < this._LOW_RPM && this.current_gear > this._LOWEST_GEAR) {
                    this.current_gear--;
                }
            }
            this.last_rpm = current_rpm;
